    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if ((sender instanceof Player)) {
            ...
            if (plugin.Funcs.getIsLoggedIn(player) && player.hasPermission("SPfB.listwp")) {
                ...
                if (args.length == 0) {
                    try {
                        plugin.Funcs.listWaypoints(player);
            ...
            else plugin.Funcs.sendSystemMessage(player, "Du bist nicht eingeloggt oder hast nicht die erforderliche Berechtigung SPfB.listwp");
        }
        if (args.length == 1) {
            if (!(sender instanceof Player) || (plugin.Funcs.getIsLoggedIn((Player)sender) && sender.hasPermission("SPfB.listallwp"))) {
                try {
                    plugin.Funcs.listWaypoints(args[0]);
                } catch (SQLException | MySqlPoolableException e) {
                    e.printStackTrace();
                }
                return true;
